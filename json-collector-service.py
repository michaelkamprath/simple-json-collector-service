...
def collect_setup_errors() -> list[str]:
    errors = []
    if not os.path.exists(DATA_FILE_DIR):
        errors.append(f"DATA_FILE_DIR '{DATA_FILE_DIR}' does not exist")
        return errors

    if not os.path.isdir(DATA_FILE_DIR):
        errors.append(f"DATA_FILE_DIR '{DATA_FILE_DIR}' is not a directory")
        return errors

    if not os.access(DATA_FILE_DIR, os.W_OK):
        errors.append(f"DATA_FILE_DIR '{DATA_FILE_DIR}' is not writable")
...
def return_health_check():
    event_time_str = time.strftime(LOG_TIME_FORMAT, time.gmtime())
    errors = collect_setup_errors()
    status_code = 200 if not errors else 500
    payload = None if not errors else {"errors": errors}
    log_request_event(event_time_str, status_code, payload)

    if not errors:
        return "Everything is ay oh kay"

    return "ERROR - " + "; ".join(errors), status_code
